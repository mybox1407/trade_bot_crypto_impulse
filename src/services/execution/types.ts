...
export interface EnsureProtectiveOrdersRequest {
  symbol: string;
  marketId: number;
  side: PositionSide;
  quantity: number;
...
export interface ExecutionService {
  openPosition(req: OpenExecutionRequest): Promise<ExecutionResult>;
  closePosition(req: CloseExecutionRequest): Promise<ExecutionResult>;
  ensureProtectiveOrders(req: EnsureProtectiveOrdersRequest): Promise<ProtectiveOrders>;
  cancelProtectiveOrders?(orders: ProtectiveOrders): Promise<void>;
  stop?(): void;
}
